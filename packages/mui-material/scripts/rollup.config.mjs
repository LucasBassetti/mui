...
const nestedFolder = {
  resolveId: (importee) => {
    if (importee.indexOf('@temp-mui/base/') === 0) {
      return resolveNestedImport('mui-base', importee);
    }

    if (importee.indexOf('@temp-mui/utils/') === 0) {
      return resolveNestedImport('mui-utils', importee);
    }

    if (importee.indexOf('@temp-mui/private-theming/') === 0) {
      return resolveNestedImport('mui-private-theming', importee);
    }

    if (importee.indexOf('@temp-mui/styled-engine/') === 0) {
      return resolveNestedImport('mui-styled-engine', importee);
    }

    if (importee.indexOf('@temp-mui/system/') === 0) {
