...
export default function findComponentJSX(j, options, callback) {
  ...
  // case 1: import ComponentName from '@temp-mui/material/ComponentName';
  // case 2: import { ComponentName } from '@temp-mui/material';
  // case 3: import { ComponentName as SomethingElse } from '@temp-mui/material';
  ...
  root
    ...
    .filter((path) =>
      path.node.source.value.match(new RegExp(`^@temp-mui/material(/${componentName})?$`)),
    )
    .forEach((path) => {
