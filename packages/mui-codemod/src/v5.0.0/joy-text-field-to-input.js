...
export default function transformer(file, api, options) {
  ...
  root
    ...
    .filter(({ node }) => {
      ...
      if (sourceVal === '@temp-mui/joy/TextField') {
        node.source.value = '@temp-mui/joy/Input';
      }

      return [
        '@temp-mui/joy', // Process only Joy UI components
        '@temp-mui/joy/TextField', // Filter default imports of components other than TextField
      ].includes(sourceVal);
    })
    .forEach((path) => {
