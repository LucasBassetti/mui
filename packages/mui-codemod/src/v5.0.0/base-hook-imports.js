...
export default function transformer(file, api, options) {
  ...
  root
    ...
    .filter((path) => {
      ...
      return sourceVal.startsWith('@temp-mui/base/') && sourceVal.match('Unstyled');
    }) // Process only Base components
    .forEach((path) => {
      // scenario 1: `@temp-mui/base/<Component>Unstyled/use<Component>`
      // e.g., `@temp-mui/base/SelectUnstyled/useSelect`
      const sourceVal = path.node.source.value;
      // replace `@temp-mui/base/SelectUnstyled/useSelect` with `@temp-mui/base/useSelect`
      if (sourceVal.replace(/@temp-mui\/base\/([a-zA-Z]+)Unstyled/, '') !== '') {
        path.node.source.value = sourceVal.replace(
          /@temp-mui\/base\/([a-zA-Z]+)Unstyled\/([a-zA-Z]+)/,
          '@temp-mui/base/$2',
        );
        return;
      }

      // scenario 2: @temp-mui/base/<Component>Unstyled
      const specifiersForHook = [];
      const filteredSpecifiers = [];
      const hookName = sourceVal.replace(/@temp-mui\/base\/([a-zA-Z]+)Unstyled/, 'use$1');
      ...
      const hookImportDeclaration = j.importDeclaration(
        specifiersForHook,
        j.literal(`@temp-mui/base/${hookName}`),
      );
