...
export default function transformer(fileInfo, api, options) {
  ...
  root
    ...
    .forEach((path) => {
      ...
      const subPackageImportMatch = importSource.match(/@temp-mui\/lab\/(.*)/);
      if (subPackageImportMatch !== null) {
        const subModule = subPackageImportMatch[1];

        if (subModule.startsWith('internal')) {
          console.warn('Imports from `@temp-mui/lab/internal` are not supported');
          return;
        }
      ...
      } else if (importSource === '@temp-mui/lab') {
        ...
        if (xImportSpecifiers.length > 0) {
          const targetPackage = isPro ? '@temp-mui/x-date-pickers-pro' : '@temp-mui/x-date-pickers';
          path.replace(
            j.importDeclaration(xImportSpecifiers, j.stringLiteral(targetPackage)),
            j.importDeclaration(labImportSpecifiers, j.stringLiteral('@temp-mui/lab')),
          );
        }
      }
    })
