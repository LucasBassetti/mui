...
if (process.env.NODE_ENV === 'test') {
  ...
  require.resolve = (source) =>
    resolve(source.replace(/^@temp-mui\/material\/modern/, '../../../mui-material/src'));
}

export default function transformer(fileInfo, api, options) {
  const j = api.jscodeshift;
  const importModule = options.importModule || '@temp-mui/material';
  const targetModule = options.targetModule || '@temp-mui/material';
