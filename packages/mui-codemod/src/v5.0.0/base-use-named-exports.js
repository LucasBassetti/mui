...
const pathMapping = {
  '@temp-mui/base/className': '@temp-mui/base/ClassNameGenerator',
};
...
/**
 * Finds the last segment of the path starting with @temp-mui/base.
 *
 * @example '@temp-mui/base/Menu' ➔ 'Menu'
 * @example '@temp-mui/base' ➔ null
 */
function getBaseImportIdentifier(path, filePath) {
  ...
  const baseImportPathMatch = source.match(/@temp-mui\/base\/(.+)/);
  ...
  const withTransformedImports = j(file.source)
    ...
    .forEach((path) => {
      ...
      path.node.specifiers = path.node.specifiers.map((specifier) => {
        ...
        // import Y from @temp-mui/base/X ➔ import { X as Y } from @temp-mui/base/X
    ...
    .forEach((path) => {
      ...
      path.node.specifiers = path.node.specifiers.map((specifier) => {
        ...
        if (specifier.exported.name === 'default') {
          // export { default } from @temp-mui/base/X ➔ export { X as default } from @temp-mui/base/X
        ...
        // export { default as Y } from @temp-mui/base/X ➔ export { X as Y } from @temp-mui/base/X
        return j.exportSpecifier.from({
