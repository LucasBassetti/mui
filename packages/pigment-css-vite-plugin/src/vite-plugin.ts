  ...
  return {
    ...
    async transform(code: string, url: string) {
      ...
      const asyncResolve = async (what: string, importer: string, stack: string[]) => {
        // @TODO - Remove this when @temp-mui/system published esm files by default at the base directory
        if (what.startsWith('@temp-mui/system') && !what.includes('esm')) {
          return what.replace('@temp-mui/system', '@temp-mui/system/esm');
        }
