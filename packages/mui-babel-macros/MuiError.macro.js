...
/**
 * Supported imports:
 * 1. bare specifier e.g. `'@temp-mui/internal-babel-macros/MuiError.macro'`
...
function muiError({ references, babel, config, source }) {
  ...
  references.default.forEach((babelPath) => {
    ...
    if (!newExpressionPath.isNewExpression()) {
      throw new MacroError(
        'Encountered `MuiError` outside of a "new expression" i.e. `new MuiError()`. Use `throw new Error(message)` over `throw MuiError(message)`.',
      );
    }
    ...
    if (formatMuiErrorMessageIdentifier === null) {
      const isBareImportSourceIdentifier = source.startsWith('@temp-mui/internal-babel-macros');
      if (isBareImportSourceIdentifier) {
        // Input: import MuiError from '@temp-mui/internal-babel-macros/MuiError.macro'
        // Outputs:
        // import { formatMuiErrorMessage } from '@temp-mui/utils';
        formatMuiErrorMessageIdentifier = helperModuleImports.addDefault(
          babelPath,
          '@temp-mui/utils/formatMuiErrorMessage',
          { nameHint: '_formatMuiErrorMessage' },
        );
      } else {
        throw new Error('Only package imports from @temp-mui/internal-babel-macros are supported');
      }
    }
